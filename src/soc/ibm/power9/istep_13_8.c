...
static void p9n_mca_scom(int mcs_i, int mca_i)
{
	...
	int vpd_idx = mranks & 2;	/* 2 when 2R, 0 when 1R */
	if (n_dimms == 2)
		vpd_idx++;
...
//~ static const uint32_t EPSILON_R_T2_LE[] = {   67,   69,   71,   74,   79,  103 };  // T2
	...
	*/
	/*
	 * FIXME: remember that VPD data layout is different, code will be slightly
	 * different when reading from PNOR (s/vpd_idx/mca_i/).
	 */
	#define F(X)	((((X) >> 4) & 0xc) | (((X) >> 2) & 0x3))
	mca_and_or(id, mca_i, 0x07010915, 0,
	           PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_RD[vpd_idx][0][0]), 3) |
	           PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_RD[vpd_idx][0][1]), 7) |
	           // PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_RD[vpd_idx][0][2]), 11) |		// 0
	           // PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_RD[vpd_idx][0][3]), 15) |		// 0
	           PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_RD[vpd_idx][1][0]), 19) |
	           PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_RD[vpd_idx][1][1]), 23) |
	           // PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_RD[vpd_idx][1][2]), 27) |		// 0
	           // PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_RD[vpd_idx][1][3]), 31) |		// 0
	           PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_WR[vpd_idx][0][0]), 35) |
	           PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_WR[vpd_idx][0][1]), 39) |
	           // PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_WR[vpd_idx][0][2]), 43) |		// 0
	           // PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_WR[vpd_idx][0][3]), 47) |		// 0
	           PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_WR[vpd_idx][1][0]), 51) |
	           PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_WR[vpd_idx][1][1]), 55));
	           // PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_WR[vpd_idx][1][2]), 59) |		// 0
	           // PPC_SHIFT(F(ATTR_MSS_VPD_MT_ODT_WR[vpd_idx][1][3]), 63));		// 0
	#undef F
...
static void seq_reset(int mcs_i, int mca_i)
{
	...
	*/
	 ...
	 *
	 * FIXME: remember that VPD data layout is different, code will be slightly
	 * different when reading from PNOR.
	 */
#define F(x)	(((x >> 4) & 0xC) | ((x >> 2) & 0x3))
