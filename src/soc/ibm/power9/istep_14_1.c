...
static void load_maint_pattern(int mcs_i, uint64_t pat)
{
	chiplet_id_t id = mcs_ids[mcs_i];
	/*
	 ...
	 *
	 * TODO: consider using 'uint64_t pat[8]' instead
	 */
	int mca_i;
	...
	for (mca_i = 0; mca_i < MCA_PER_MCS; mca_i++) {
		...
		for (i = 0; i < 16; i++) {
			...
			mca_write(id, mca_i, 0x07010A2A, pat);
			/*
...
static void init_mcbist(int mcs_i)
{
	...
	for (i = 0; i < 9; i++) {
		write_scom_for_chiplet(id, 0x070123BE + i, 0);
	}

	/* TODO: random seeds */

	/*
	 * Maintenance data pattern - what is the difference between this and data
	 * pattern above? Is this used for targeted mode and the previous one for
	 * broadcast? Do they differ in ECC handling?
	 */
	/* TODO: different patterns, they can have more than 8 bytes */
	load_maint_pattern(mcs_i, 0);
...
{
	uint16_t test = WRITE | (port_dimm << 9) | RAND_FWD_MAINT | ECC_MODE;
